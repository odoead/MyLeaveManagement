    ...
    internal class MockLeaveAllocationRepository
    {
        public static Mock<ILeaveAllocationRepository> GetMock()
        {
            ...
            mock.Setup(m => m.CheckAllocationAsync(It.IsAny<int>(), It.IsAny<string>()))
                .ReturnsAsync(
                    (int leavetypeid, string emloyeeid) =>
                        EmptyAllocations.Exists(q =>
                            q.EmployeeId == emloyeeid && q.LeaveTypeId == leavetypeid
                        )
                );
            mock.Setup(m => m.CreateAsync(It.IsAny<LeaveAllocation>())).ReturnsAsync(true);
            mock.Setup(m => m.DeleteAsync(It.IsAny<LeaveAllocation>())).ReturnsAsync(true);
            mock.Setup(m => m.FindByIdAsync(It.IsAny<int>()))
                .ReturnsAsync((int T) => allocations.FirstOrDefault(a => a.Id == T));
            mock.Setup(m => m.GetAllAsync()).ReturnsAsync(allocations);
            ...
            mock.Setup(m =>
                    m.GetLeaveAllocationByEmloyeeAndTypeAsync(It.IsAny<string>(), It.IsAny<int>())
                )
                .ReturnsAsync(
                    (string id, int type) =>
                        allocations.FirstOrDefault(a =>
                            a.EmployeeId == id && a.LeaveType.Id == type
                        )
                );
            ...
            mock.Setup(m => m.UpdateAsync(It.IsAny<LeaveAllocation>())).ReturnsAsync(true);
            return mock;
        }
    }
}
