...
namespace Tests.mocks
{
    internal class MockLeaveRequestRepository
    {
        public static Mock<ILeaveRequestRepository> GetMock()
        {
            ...
            mock.Setup(m => m.CreateAsync(It.IsAny<LeaveRequest>())).ReturnsAsync(true);
            mock.Setup(m => m.DeleteAsync(It.IsAny<LeaveRequest>())).ReturnsAsync(true);
            mock.Setup(m => m.FindByIdAsync(It.IsAny<int>()))
                .ReturnsAsync((int T) => requests.FirstOrDefault(a => a.Id == T));
            mock.Setup(m => m.GetAllAsync()).ReturnsAsync(requests);
            mock.Setup(m => m.GetRequestsByEmployeeAsync(It.IsAny<string>()))
                .ReturnsAsync(
                    (string id) => requests.Where(a => a.RequestingEmpoyeeId == id).ToList()
                );
            ...
            mock.Setup(m => m.UpdateAsync(It.IsAny<LeaveRequest>())).ReturnsAsync(true);
            return mock;
        }
    }
}
